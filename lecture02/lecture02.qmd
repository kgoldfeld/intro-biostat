...
We'll move from **describing variability in a sample** to asking what a sample
can tell us about a population.

- How would our results vary if we took another sample?
- How can we determine whether an observed result is unexpected given ordinary
  sampling variation?
- How do hypothesis tests use this idea to quantify uncertainty?
- How can we compare means in several common study designs?

These ideas provide the foundation for statistical inference.
...
ggplot(data.table(bw_means), aes(x = bw_means)) +
  ...
  labs(
    title = 
      "Sampling Distribution of the Mean Birth Weight",
    x = "Estimated Mean",
    y = "Count"
  )
```
...
ggplot(data.table(x), aes(x = x)) +
  ...
  ggtitle(
    "Single sample from the Poisson distribution"
  )
```
...
ggplot(data.table(means), aes(x = means)) +
  ...
  labs(
    ...
    subtitle = 
      "Curve shows the Normal approximation predicted by the CLT",
    x = "Sample mean",
    y = "Density"
  )
```
...
ggplot(
  ...
  labs(
    title = 
      "Observed Mean in the Null Distribution",
    x = "Sample mean",
    y = "Count"
  )
```
...
p + 
  ...
  labs(
    title = 
      "Extreme values for one-sided hypothesis test",
    x = "Sample Mean",
    y = "Count"
  ) +
  scale_fill_manual(
...
p2 +
  ...
  labs(
    title = 
      "Extreme values for two-sided hypothesis test",
    x = "Sample Mean",
    y = "Count"
  ) +
  scale_fill_manual(
    values = 
      c("TRUE" = "steelblue", "FALSE" = "grey85"),
    guide = "none"
  )
```
...
(p_full | p_zoom) +
  plot_annotation(
    title = 
      "Simulated sampling distributions of the standardized statistics"
  ) +
  plot_layout(
    axes = "collect", 
    axis_titles = "collect"
...
location <- xbar - mu
scale <- sd(birth_weight) / sqrt(length(birth_weight)) 
t_stat <- location / scale 
...
> Pairing can improve precision because differences between individuals that affect both measurements are removed when we calculate the within-person change.
...
### Advantage of the paired analysis

If we inspect the standard deviation of the individual *before* and *after* measurements and the standard deviation of the *change*, we can see the advantage of this design:
...
> In our simulation, each person's baseline grip strength contributes to both measurements. When we calculate the within-person difference, that between-person variability cancels out. The resulting reduction in variability allows us to estimate the average change more precisely.
