...
For each outcome $\large j$:

$$\Large
H_{0j}: \mu_{j1} - \mu_{j0} = 0,
\qquad j=1,\ldots,20$$

where $\large \mu_{j1}$ and $\large \mu_{j0}$ are the population means for outcome $j$ in the treatment and control groups.

Suppose **all 20 null hypotheses are true** — treatment has no effect on any of the outcomes.

For any one test with $\large \alpha=0.05$, the probability of a **Type I error** is 5%.
...
- we test each outcome using $\large \alpha=0.05$
...
Instead of testing each hypothesis using $\large \alpha=0.05$, divide $\large \alpha$ by the number of tests:

$$\Large
\alpha^*=\frac{\alpha}{m}$$

where $\large m$ is the number of tests.
...
We now reject an individual null hypothesis only when $\large p < 0.0025$:
...
The $\large \leq$ is important:

- Bonferroni guarantees that the FWER is **no greater than 5%** — it does not have to be exactly 5%.
- Unlike $\large 1-(0.95)^{20}$, Bonferroni **does not require the tests to be independent**.
...
- compare the smallest p-value with $\large 0.05/5$
- if rejected, compare the next with $\large 0.05/4$
...
d_result <- data.table(effect = effect, reject_null = as.integer(p_values < 0.05))
```
...
```{webr-r}
FDP <- ifelse(total_discoveries == 0, 0, false_discoveries / total_discoveries)
FDP
```
...
sim_FDP <- replicate(n_sims, {
  ...
  ifelse(total_discoveries == 0, 0, false_discoveries / total_discoveries)
})
...
d_result_BH <- data.table(effect = effect, reject_null = as.integer(p_values_BH < 0.05))

false_discoveries <- d_result_BH[effect == 0, sum(reject_null)]
true_discoveries <- d_result_BH[effect != 0, sum(reject_null)]
total_discoveries <- d_result_BH[, sum(reject_null)]
...
sim_FDP_BH <- replicate(n_sims, {
  ...
  false_discoveries <- d_results[effect == 0, sum(reject_null)]
  total_discoveries <- d_results[, sum(reject_null)]
...
where $\large i$ is the position of the p-value in the ordered list.

For five tests, the thresholds are:

| $\large i$ |       BH threshold |
|-----------:|-------------------:|
|          1 | $(1/5)(0.05)=0.01$ |
|          2 | $(2/5)(0.05)=0.02$ |
|          3 | $(3/5)(0.05)=0.03$ |
...
The largest p-value satisfying the criterion is $\large p_{(3)}=0.027$, so BH rejects the first **three** null hypotheses.
...
- a baseline health score $\large X$, observed for everyone
- mean outcome of 50 for a participant with $\large X=0$
...
dd[, outcome_obs := fifelse(missing == 1, NA_real_, outcome)]
...
sim_means <- replicate(n_sims, {
  ...
  d_sim[, outcome_obs := fifelse(missing == 1, NA_real_, outcome)]
